...
fn tree_dfs(tree: &DynTree<String>) -> i64 {
    let data: Vec<_> = tree.root().indices::<Dfs>().collect();
    data.into_par()
        .map(|x| tree.node(&x).data().parse::<usize>().unwrap())
        .map(|x| fibonacci(x as i64 % 500))
        .sum()
    // let x: &NodeIdx<Dyn<String>> = &data[0];
    // let par = data
    //     .par()
    //     .map(|idx| idx.clone())
    //     .map(|x| tree.root().data());

    // let x: Vec<_> = par.collect();
    // let y = &x[0];
...
fn tree_dfs_parallel(tree: &DynTree<String>) -> i64 {
    let node = tree.root();
    node.walk::<Dfs>()
        .iter_into_par()
        .map(|x| x.parse::<usize>().unwrap())
...
fn bench(c: &mut Criterion) {
    ...
    for n in &treatments {
        ...
        group.bench_with_input(
            BenchmarkId::new("Tree::root().walk_par::<Dfs>()", n),
            n,
            |b, _| {
                let result = tree_dfs_parallel(&data);
                assert_eq!(result, expected);
                b.iter(|| tree_dfs_parallel(&data))
            },
        );
