...
fn dfs_iter_ref_empty() {
    ...
    let mut iter = unsafe { DfsIterMut::<_, Auto, _, Val, _, &mut i32>::from((&tree.0, iter)) };
...
fn dfs_iter_mut_val() {
    ...
    let iter = unsafe { DfsIterMut::<_, Auto, _, Val, _, &mut i32>::from((&tree.0, iter)) };
    ...
    let ptr = root.node_ptr().clone();
    let iter = DfsIterPtr::<_, Val, _>::from((&mut stack, ptr));
    let iter = DfsIterRef::<_, Auto, _, Val, _, &i32>::from((root.col(), iter));
    assert_eq!(
...
fn dfs_iter_mut_depth() {
    ...
    let iter = unsafe { DfsIterMut::<_, Auto, _, DepthVal, _, &mut i32>::from((&tree.0, iter)) };
...
fn dfs_iter_mut_sibling() {
    ...
    let iter =
        unsafe { DfsIterMut::<_, Auto, _, SiblingIdxVal, _, &mut i32>::from((&tree.0, iter)) };
...
fn dfs_iter_mut_depth_sibling() {
    ...
    let iter =
        unsafe { DfsIterMut::<_, Auto, _, DepthSiblingIdxVal, _, &mut i32>::from((&tree.0, iter)) };
