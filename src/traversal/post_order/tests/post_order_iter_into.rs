...
fn post_order_into_iter_partially_used(use_iter: UseIter) {
...

    {
        ...
        let ptr = n3.node_ptr();
        let iter = PostOrderIterPtr::<_, Val, _>::from((Vec::default(), ptr));
        {
            let mut iter = unsafe {
                PostOrderIterInto::<_, Auto, SplitRecursive, Val, _>::from((&mut tree.0, iter, ptr))
            };
...
fn post_order_into_iter_depth() {
...

    {
        ...
        let n3 = root.get_child(1).unwrap();
        let ptr = n3.node_ptr();
        let iter = PostOrderIterPtr::<_, DepthVal, _>::from((Vec::default(), ptr));
        let iter = unsafe {
            PostOrderIterInto::<_, Auto, SplitRecursive, DepthVal, _>::from((
                &mut tree.0,
                iter,
...
fn post_order_into_iter_sibling_idx() {
...

    {
        ...
        let n3 = root.get_child(1).unwrap();
        let ptr = n3.node_ptr();
        let iter = PostOrderIterPtr::<_, SiblingIdxVal, _>::from((Vec::default(), ptr));
        let iter = unsafe {
            PostOrderIterInto::<_, Auto, SplitRecursive, SiblingIdxVal, _>::from((
                &mut tree.0,
                iter,
