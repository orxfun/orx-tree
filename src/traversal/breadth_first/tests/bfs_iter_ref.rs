...
fn bfs_iter_ref_empty() {
    ...
    let mut iter = BfsIterRef::<_, _, _, Val, _, NodePtr<_>>::from((&tree.0, iter));
...
fn bfs_iter_for<O: Over<Dyn<i32>>>() {
    ...
    let iter = BfsIterRef::<_, _, _, Val, _, Item<'_, O>>::from((root.col(), iter));
...
fn bfs_iter_ref_depth() {
    ...
    let iter = BfsIterRef::<_, _, _, DepthVal, _, &i32>::from((root.col(), iter));
    assert_eq!(
...
fn bfs_iter_ref_sibling() {
    ...
    let iter = BfsIterRef::<_, _, _, SiblingIdxVal, _, &i32>::from((root.col(), iter));
    assert_eq!(
        iter.map(|x| x.0).collect::<Vec<_>>(),
...
fn bfs_iter_ref_depth_sibling() {
    ...
    let iter = BfsIterRef::<_, _, _, DepthSiblingIdxVal, _, &i32>::from((root.col(), iter));
    assert_eq!(
