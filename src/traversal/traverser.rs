...
fn xyz2() {
    ...
    let mut tree = DaryTree::<4, i32>::new(1);
    ...
    let mut n3 = tree2_id3.node_mut(&mut tree);
    ...
    let n3 = tree2_id3.node_mut(&mut tree);
    let vals: Vec<_> = tr.into_iter(n3).collect();
    ...
    let remaining: Vec<_> = tr.iter(&tree.root().unwrap()).copied().collect();
    assert_eq!(remaining, [1, 2, 4, 5, 8]);
}
...
fn abc() {
    ...
    let mut n2 = id2.node_mut(&mut tree);
    let [id3, id4] = n2.grow([3, 4]);

    id4.node_mut(&mut tree).push(5);

    let root = tree.root().unwrap();
    let vals: Vec<_> = tr.iter(&root).copied().collect();
    assert_eq!(vals, [1, 2, 3, 4, 5]);

    let nd = id4.node_mut(&mut tree);
    let vals: Vec<_> = tr.into_iter(nd).collect();
    assert_eq!(vals, [4, 5]);
