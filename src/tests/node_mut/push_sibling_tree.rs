...
#[should_panic]
fn push_sibling_tree_cloned_to_root(side: Side) {
    ...
    let other = DynTree::new(42.to_string());
...
#[test_matrix([Side::Left, Side::Right])]
#[should_panic]
fn push_sibling_tree_copied_to_root(side: Side) {
...
#[test_matrix([Side::Left, Side::Right])]
#[should_panic]
fn push_sibling_tree_moved_to_root(side: Side) {
    let mut tree = DynTree::new(0);
    let [id1, _] = tree.root_mut().push_children([1, 2]);
    let [_id3, _id4, _id5] = tree.node_mut(id1).push_children([3, 4, 5]);
    let id_dst = tree.root().idx();
